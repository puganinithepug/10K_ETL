...
I built a scalable, Dockerized SEC 10-K Extract-Transform-Load agent that reduces manual effort by 92%—from 2 hours to 10 minutes. The agent automates searching, parsing, and extraction of XML filings into key financial statements. The agentic flow starts with a data preprocessing pipeline that pre-sorted entries by grouping them according to context-reference and label entry features, enabling discovery of the sets representing balance sheet versus income and cashflow statement data. I used open coding to create the training dataset for the ML component, which granularly categorizes the pre-sorted data by statement type and category, outputting organized CSVs for each statement. The agent combines TF-IDF and semantic clustering pipelines. Incorporating unsupervised learning addressed the possibility of the data being incomplete. Training data is enriched with high-confidence classifications to improve accuracy over time. 

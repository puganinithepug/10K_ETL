...
import NLP_Classifier_W5 as NLP
...
def parse_args():
    parser = argparse.ArgumentParser(description="ML or NLP?")
    parser.add_argument("--type", help="ML or NLP", default=None)

    args, _ = parser.parse_known_args()

    while not args.type:
        answer = input("Type: ML or NLP ")
        if "ML" in answer:
            args.type=0
            break
        elif "NLP" in answer:
            ...
            print("Error: select classification engine")
    return args
...
def main():
    ...
    new_dir_W5 = "nlp_classified_inc_cf_bal_data_W5"
    ...
    files_W5 = [f"nlp_bal_sheet_{ticker}.csv", f"nlp_inc_cf_{ticker}.csv", f"nlp_inc_{ticker}.csv", f"nlp_cf_{ticker}.csv"]

    organize = [(new_dir_W1, files_W1), (new_dir_W2, files_W2), (new_dir_W3, files_W3), (new_dir_W4, files_W4), (new_dir_W5, files_W5)]
